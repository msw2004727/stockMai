...
## [2026-03-03 14:46] AI分析頁閱讀順序重排與卡片間距優化
- 完成事項：
  - 將策略決策大欄位（策略決策/構成分數/決策理由）移至「你想要 AI 著重的分析重點」下方，直接整合到 `AIPanel`。
  - 「AI短線分析（1~5 個交易日）」更名為「AI依據資料分析」，並移除重複的小標題文字。
  - 將「展開/收合AI反饋數據」按鈕移到 AI 結果區最下方。
  - 新增 `stack-block` / `detail-stack` 間距規則，讓市場情緒、技術指標快照與各模型卡片之間保留明確留白，不再黏在一起。
  - 前端驗證：`npm run build` 通過。
- 目前進度：Done
- 下一步：請在手機版實機驗收新版排序（分析重點→策略決策→AI共識→AI依據資料分析→AI反饋收折）與卡片留白是否符合預期。
- 備註（可選）：本回合未改動後端 API。
## [2026-03-03 14:47] AI分析版面重排提交與推送
- 完成事項：
  - 將策略決策區整合到 AI 分析流程中，位置調整到分析重點欄位下方。
  - 完成 AI 依據資料分析命名調整、AI 反饋收折按鈕移到最下方、卡片間距優化。
  - 本回合執行 commit + push。
- 目前進度：Done
- 下一步：請在手機版驗收新版流程操作順序與卡片間距，若要再微調字級或間距可再下一回合調整。
- 備註（可選）：本回合為前端結構與樣式調整。
## [2026-03-03 15:25] 快速查詢改為前一交易日三大排行可行性評估
- 完成事項：
  - 盤點現況：前端 `QuotePanel` 的快速查詢目前為固定 6 檔靜態清單，後端尚無 top volume / top gainers / top losers 專用 API。
  - 確認資料面：現有 `stock_daily_prices` 可支援排行查詢，但若要「全市場前六」需先有完整市場日資料（非僅使用者查過的個股）。
  - 產出實作方向：建議新增市場排行 API 與每日快照/快取機制，再由前端改成三欄獨立區塊展示。
- 目前進度：In Progress
- 下一步：與使用者確認排行範圍（上市/上櫃/ETF 是否納入）與可接受資料更新時點（收盤後批次或隔日固定時間）。
- 備註（可選）：本回合為方案評估與風險說明，未修改程式碼。
## [2026-03-03 15:34] 完整版計劃第1階段：市場排行 API 與行情快速查詢改版
- 完成事項：
  - 新增後端 API：`GET /stocks/movers?limit=6`，回傳前一交易日三分類排行（成交量/漲幅/跌幅）與交易日資訊。
  - 後端分層新增：`movers_repository.py`（DB 排行查詢）與 `movers_service.py`（服務封裝與錯誤處理），`routes.py` 新增路由。
  - 前端行情頁快速查詢改為三個獨立分類欄位，資料來源改為 `/stocks/movers`，點擊項目可直接帶入查價。
  - 串接資料流：`api.js`、`useQuoteHistory`、`App.vue`、`MarketView.vue`、`QuotePanel.vue`、`styles.css`。
  - 驗證：frontend `npm run build` 通過；backend `py_compile`（新檔與路由）通過。
- 目前進度：Done
...
## [2026-03-03 17:45] 三大分類改為上下排列
- 完成事項：
  - 將行情快速查詢的三大分類容器由橫向三欄改為單欄堆疊，呈現為上到下排列。
  - 各分類內的小卡仍維持每列 3 格、共 2 列的密度配置。
...
## [2026-03-03 18:43] AI 分析加載彈窗文案替換
- 完成事項：
  - 將執行 AI 分析時的加載彈窗文案，由「AI 分析中，請稍候...」改為完整風險提醒文案。
  - 保持行情查詢加載文案與顯示邏輯不變（`行情查詢中，請稍候...`）。
  - 前端驗證：`frontend` 執行 `npm run build` 通過。
- 目前進度：Done
- 下一步：請實機點擊「執行 AI 分析」確認彈窗文案顯示完整且可讀性符合預期。
- 備註（可選）：本回合未執行 commit + push。
## [2026-03-03 18:54] AI 反饋改為向下展開並新增多空分析師欄位
- 完成事項：
  - 將「展開AI反饋數據」按鈕位置移到詳細內容上方，改為按鈕點擊後往下展開內容。
  - 在「AI 共識結果」下方新增「多空分析師觀點」卡片，包含：
    - `看多分析師`：以趨勢、動能、情緒與風險控管角度生成偏多敘述。
    - `看空分析師`：以壓力、背離、波動與防守角度生成偏空敘述。
    - 分隔線後新增 `輕鬆總結`（約 100 字）統整多空重點。
  - 新增對應樣式：多空欄位底色、文字排版、分隔線與總結樣式。
  - 前端驗證：`frontend` 執行 `npm run build` 通過。
- 目前進度：Done
- 下一步：請在 AI 分析頁實機檢查閱讀順序與文字密度；若要再調整「更長/更短」敘述可再微調模板。
- 備註（可選）：本回合未執行 commit + push。
## [2026-03-03 18:55] AI依據資料分析改放入收折區塊
- 完成事項：
  - 將 `AI依據資料分析` 欄位從常駐區塊移入 `展開AI反饋數據` 收折內容內。
  - 目前流程改為：先顯示共識與多空分析師觀點，點擊展開後才看到 AI 依據資料分析、市場情緒、技術指標與模型回饋。
  - 前端驗證：`frontend` 執行 `npm run build` 通過。
- 目前進度：Done
- 下一步：請你在 AI 頁面點擊展開/收合確認閱讀順序是否符合預期。
- 備註（可選）：本回合未執行 commit + push。
## [2026-03-03 19:08] AI多空字數精簡與PC排版優化
- 完成事項：
  - 將「看多分析師 / 看空分析師」內容精簡為約 250 字左右的版本，保留指標、情緒與風控觀點。
  - 在「展開AI反饋數據」區塊底部新增置中提醒卡片：`提醒：目前測試版參考資料不多，分析僅供參考。`
  - 將 AI 上方控制區改為 `ai-control-grid`，在 PC（>=1024px）採兩欄排版（分析標的 / AI核心），並讓「分析重點」橫跨全寬。
  - 縮減「你想要 AI 著重的分析重點」欄位留白：`textarea` 由 `rows=3` 改 `rows=2`，最小高度由 `84px` 降至 `64px`（PC 再降至 `58px`）。
  - 前端驗證：`frontend` 執行 `npm run build` 通過。
- 目前進度：Done
...
## [2026-03-03 20:06] 完成AI模型技術分析功能提交與推送
- 完成事項：
  - 將 AI 模型技術分析相關前後端與測試變更提交為 commit `2531043`。
  - 已推送至 `origin/main`，雲端可直接拉到最新版本。
- 目前進度：Done
- 下一步：在雲端環境手動執行一次 AI 分析，確認「收合AI反饋數據」底部 5 項技術指標顯示正常。
- 備註（可選）：若某 provider 未回傳完整 usage，Token/電力/碳排會顯示 N/A（設計預期）。
## [2026-03-03 20:14] 修正AI加載跑馬燈為無縫循環
- 完成事項：
  - 將 AI 加載彈窗的跑馬燈改為「雙軌重複內容」結構，避免原本跑到終點瞬間跳回起點的斷裂感。
  - 調整動畫位移為 `translateX(-50%)`，實現連續循環視覺。
  - 前端 `npm run build` 驗證通過。
- 目前進度：Done
- 下一步：你可在前端觸發一次 AI 分析，確認「AI努力分析中」已是平滑不斷循環。
- 備註（可選）：本回合僅前端動畫與結構調整，未更動後端邏輯。
## [2026-03-03 20:16] 討論AI模型技術分析欄位防換行設計
- 完成事項：
  - 確認需求是長字串（Token明細、公式）在手機與PC皆不換行，先討論不實作。
  - 擬定設計方向：固定欄寬標題 + 單行資訊列 + 橫向滑動容器（確保不換行且不截斷）。
- 目前進度：In Progress
- 下一步：待使用者確認版型方案後再實作CSS與結構調整。
- 備註（可選）：若禁止橫向滑動，需改短文案/縮寫，否則小螢幕無法同時保留完整字串且不換行。
## [2026-03-03 20:18] AI模型技術分析欄位改為單行不換行版型
- 完成事項：
  - AI模型技術分析改為可橫向滑動容器，所有列與公式採單行 `nowrap` 顯示，避免長字串換行。
  - 列項目改成固定雙欄寬度（指標名稱 / 數值），並將 Token 詳細數據改為膠囊樣式（總計、輸入、輸出）提高可讀性。
  - 前端 `npm run build` 驗證通過。
- 目前進度：Done
- 下一步：部署後用手機與PC各測一次 AI 分析，確認技術分析區可連續單行顯示且可左右滑動檢視完整內容。
- 備註（可選）：本次僅調整前端呈現，未改後端計算邏輯。
## [2026-03-03 20:38] 精簡AI模型技術分析欄位並縮窄寬度
- 完成事項：
  - 刪除「公式：...」那一行計算公式文字。
  - 縮窄技術分析欄位寬度與間距（grid 最小寬、欄位寬、字體、chip 尺寸），降低對整體版面影響。
  - 保留橫向滑動能力與不換行設定，確保長字串仍可完整查看。
  - 前端 `npm run build` 驗證通過。
- 目前進度：Done
- 下一步：請在手機版檢查 AI 模型技術分析區，確認欄位寬度與滑動體驗符合預期。
- 備註（可選）：本回合未執行 commit + push，待使用者確認後可直接提交。
## [2026-03-03 21:35] AI模型技術分析改為上下排列膠囊樣式
- 完成事項：
  - 移除「不換行 + 可橫向滑動」機制，AI模型技術分析改為每列上下排列（標題在上、數據膠囊在下）。
  - 第 3 項 Token 數據改為同列多膠囊（總計、輸入、輸出）顯示。
  - 前端 `npm run build` 驗證通過。
- 目前進度：Done
...
## [2026-03-04 12:38] Phase 4：重點區塊自動展開與新鮮度排序
- 完成事項：
  - 前端 `StockIntelPanel` 新增排序與優先級邏輯：依 `freshness + status` 自動決定卡片顯示順序（新鮮且可用資料優先）。
  - 依區塊優先級自動控制 `details` 開合：overview/deep/status 會預設展開最關鍵的區塊，過舊或異常資料集中於狀態區提示。
  - 「資料可用性與來源」表格改為排序後輸出，先顯示較新且可用的資料集，再往後是延遲/限制/錯誤資料。
  - 驗證通過：`frontend npm run build`、`py_compile`。
- 目前進度：Done
...
## [2026-03-04 13:57] 手機版三大法人表格可左右滑動與法人中文化
- 完成事項：
  - `StockIntelPanel` 新增 `investorLabel` 映射，將 `foreign / dealer / investment_trust` 顯示為中文（外資 / 自營商 / 投信）。
  - 三大法人表格改為手機友善橫向滑動：加入專用 class，強化 `overflow-x` 觸控滑動行為。
  - 三大法人表格在手機版提高 `min-width`（560px）以確保可左右滑移；平板以上回到 420px。
  - 驗證：`frontend npm run build` 通過。
- 目前進度：Done
- 下一步：請在手機實機或瀏覽器行動裝置模式確認「三大法人」區塊可左右滑動，且名稱顯示為外資/投信/自營商。
- 備註（可選）：本回合僅調整前端顯示與互動，不影響後端 API。
## [2026-03-04 13:59] 提交手機版法人表格滑動與中文名稱調整
- 完成事項：
  - 將手機版三大法人表格加入橫向滑動能力，避免小螢幕欄位擠壓。
  - 法人名稱改為中文顯示（外資/投信/自營商）。
  - 完成建置驗證並提交、推送到遠端。
- 目前進度：Done
- 下一步：線上環境確認手機版三大法人表格滑動手感與欄寬是否符合預期。
- 備註（可選）：本回合僅前端顯示調整，無 API 介面變更。
## [2026-03-04 14:06] 行情查詢載入彈窗改為無接縫跑馬燈並更新文案
- 完成事項：
  - `App.vue` 將行情查詢 loading 文案改為「您所查詢的股價行情正在資料庫查詢中，請稍後~」。
  - loading overlay 跑馬燈改為共用模式：AI 與行情查詢皆使用同一套無接縫跑馬燈呈現（行情查詢也會滾動顯示）。
  - 保留原本 AI 分析的跑馬燈與提醒訊息行為。
  - 驗證：`frontend npm run build` 通過。
- 目前進度：Done
- 下一步：請在前端實際觸發行情查詢，確認彈窗跑馬燈與新文案顯示符合預期。
- 備註（可選）：本回合僅調整前端 loading 呈現，不影響後端 API。
## [2026-03-04 14:07] 提交行情查詢跑馬燈與文案調整
- 完成事項：
  - 將行情查詢 loading 彈窗改為無接縫跑馬燈呈現，與 AI 載入樣式一致。
  - 文案更新為「您所查詢的股價行情正在資料庫查詢中，請稍後~」。
  - 完成程式提交並推送到 `main`。
- 目前進度：Done
- 下一步：線上實測行情查詢 loading 視覺與文案是否符合預期。
- 備註（可選）：本回合僅前端顯示調整，不影響 API。
## [2026-03-04 14:15] 強化 intel 錯誤判讀：可區分權限/無資料/來源錯誤
- 完成事項：
  - 後端 `intel_provider` 新增來源嘗試鏈（attempts）彙整，當官方與 FinMind 都失敗時會保留雙方結果，並回傳合併訊息，避免只看到單一錯誤。
  - 失敗狀態決策調整：若 fallback 有 `restricted`，最終狀態會標成 `restricted`，可直接看出權限問題。
  - 後端 `intel_mapper` 新增 `diagnosis`（code/label/detail）與 `attempts` 欄位，分類為 `可用/權限限制/無資料/來源連線失敗/來源未實作/來源錯誤`。
  - 前端 `StockIntelPanel` 狀態表新增「判讀」欄位與更清楚「細節」欄位，會顯示來源鏈摘要（例如 `TWSE:錯誤 / FinMind:權限限制`）。
  - 前端樣式補強：細節欄位支援換行，避免長訊息難讀。
  - 新增測試 `backend/tests/test_stock_intel_mapper.py`，並更新 `test_stock_intel_provider.py` 以覆蓋新合併規則。
  - 驗證：`py_compile` 通過、`python -m unittest backend.tests.test_stock_intel_provider backend.tests.test_stock_intel_mapper` 通過、`frontend npm run build` 通過。
- 目前進度：Done
- 下一步：部署後以你目前標的再看 `資料可用性與來源`，確認每個資料集都有明確判讀（權限限制/無資料/來源錯誤）。
- 備註（可選）：本機 `backend.tests.test_stock_intel_service` 仍受 `pydantic_settings` 缺失影響，未在本回合執行。
## [2026-03-04 14:17] 調整 loading 跑馬燈速度與字體，行情下方文字留空
- 完成事項：
  - `styles.css` 將 loading 跑馬燈速度放慢 50%（9s -> 13.5s）。
  - `styles.css` 將跑馬燈文字字體放大 50%（0.82rem -> 1.23rem）。
  - `App.vue` 讓行情查詢 loading 彈窗下方文字不顯示（留空），但彈窗與跑馬燈仍正常顯示。
  - 驗證：`frontend npm run build` 通過。
- 目前進度：Done
- 下一步：請在實機確認跑馬燈視覺大小與速度是否符合你要的體感。
- 備註（可選）：目前工作樹仍含先前尚未提交的後端/前端變更，本回合未執行 commit。
## [2026-03-04 14:20] 提交並推送目前整批功能調整
- 完成事項：
  - 彙整目前未提交變更（intel 錯誤判讀強化、狀態表可讀性提升、loading 跑馬燈速度與字級調整）進行版本提交。
  - 推送至遠端 `main`，確保雲端與本機一致。
- 目前進度：Done
- 下一步：到線上環境驗證資料集判讀欄位與跑馬燈體感是否符合預期。
- 備註（可選）：本回合依使用者要求執行 commit + push。
## [2026-03-04 14:26] ?? CI ???WORKLOG ???? UTF-8
- ?????
  - ?? GitHub ????`backend-tests` ?? `Validate WORKLOG encoding (UTF-8)`?`pages build` ??????????
  - ? `WORKLOG.md` ??????UTF-8 + CP950???? UTF-8?? BOM??
  - ???????? CI ???
- ?????Done
- ????push ??? `backend-tests` ? `pages build and deployment / build` ?????
- ??????????? `WORKLOG.md` ????? UTF-8 ??????
